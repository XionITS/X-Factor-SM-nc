...
def sec_asset_paging(request):
    ...
    # security = Xfactor_Security.objects.filter(computer_id__in=user.values('computer_id'))
    # user = user.filter(computer_id__in=security.values('computer_id'))
    # security_instance = Xfactor_Security.objects.get(pk=user.values('computer_id'))
