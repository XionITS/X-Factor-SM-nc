...
//			{ data: 'chassistype', title: '구분', searchable: true },
			...
			{ data: 'hw', title: '하드웨어 목록', searchable: true, orderable: false},
...
//			{ data: 'hw_gpu', title: 'VGA', searchable: true },
		...
		language: {
			"decimal": "",
			"info": "전체 _TOTAL_건 <button class='btn btn-outline-info me-1px' type='button' id='hw_all_select'>필터된 자산 그룹 생성</button>",
			//"info": "전체 _TOTAL_건 ",
			"infoEmpty": "데이터가 없습니다.",
...
$('#search-input-hs').on('keyup', function(event) {
    ...
    $(document).on('click', '#hw_all_select', function() {
        $.ajax({
            url: 'hw_select_all/',  // AJAX 요청을 보낼 URL (해당 URL에 파이썬 코드를 작성해야 합니다)
            type: 'POST',
            data: {
...
//			{ data: 'chassistype', title: '구분', searchable: true },
            ...
            { data: 'ip_address', title: 'IPv4' , searchable: true},
            { data: 'mac_address', title: 'MAC' , searchable: true},
            { data: 'sw', title: '소프트웨어 목록', searchable: true, orderable: false},
    //			{ data: 'hw_mb', title: 'Mainboard', searchable: true },
    //			{ data: 'hw_ram', title: 'RAM', searchable: true },
    //			{ data: 'hw_disk', title: 'DISK', searchable: true },
    //			{ data: 'hw_gpu', title: 'VGA', searchable: true },
...
$(document).on('click', '#sw_all_select', function() {
        $.ajax({
            url: 'sw_select_all/',  // AJAX 요청을 보낼 URL (해당 URL에 파이썬 코드를 작성해야 합니다)
            type: 'POST',
            data: {
                data: sw_asset_list_Data.ajax.params(),  // 데이터 테이블의 현재 요청에 대한 파라미터 정보를 전달합니다
                filter: {
                    column: $('#column-dropdown').data('column'),
//                    orderColumn: sw_asset_list_Data.order()[0][0],
//                    orderDir: sw_asset_list_Data.order()[0][1],
                    value: $('#search-input-hs').val(),
                    value2: $('#hs_asset_list_filter input[type="search"]').val(),
                    regex: false
                }
