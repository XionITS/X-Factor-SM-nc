...
//		],
        columnDefs: [
		    ...
		    {targets: 7, width: "7%", className: 'text-center new-text-truncate flex-cloumn align-middle', render: function(data, type, row) {return '<span title="'+row.ip_address+'" data-toggle="tooltip">'+data+'</span>'}},
            {
                ...
                render: function (data, type, row) {
                    var processedData = data === "unconfirmed" || data === "N/A on Mac" ? "" : data;
                    // var date = new Date(data); // data 값을 Date 객체로 변환합니다.
                    // var newFormat = date.toLocaleDateString('ko-KR');
                    return '<span data-toggle="tooltip">' + processedData + '</span>'
                }
            },
            {
                targets: 9,
    ...
    $(document).on('click', '#hw_pur_all_select', function() {
        $.ajax({
            url: 'hw_pur_select_all/',  // AJAX 요청을 보낼 URL (해당 URL에 파이썬 코드를 작성해야 합니다)
            type: 'POST',
            data: {
                data: hw_pur_asset_list_Data.ajax.params(),  // 데이터 테이블의 현재 요청에 대한 파라미터 정보를 전달합니다
                filter: {
...
//		],
    ...
    $(document).on('click', '#sw_pur_all_select', function() {
        $.ajax({
            url: 'sw_pur_select_all/',  // AJAX 요청을 보낼 URL (해당 URL에 파이썬 코드를 작성해야 합니다)
            type: 'POST',
            data: {
                data: sw_pur_asset_list.ajax.params(),  // 데이터 테이블의 현재 요청에 대한 파라미터 정보를 전달합니다
                filter: {
                    column: $('#column-dropdown').data('column'),
//                    orderColumn: hs_asset_list_Data.order()[0][0],
//                    orderDir: hs_asset_list_Data.order()[0][1],
