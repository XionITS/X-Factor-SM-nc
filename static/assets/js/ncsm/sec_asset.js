...
var checkedItems = {};
var showButtonsec = '';
var dataElement = document.getElementById('deploysec')
...
var sec_asset_list = function () {
    var sec_asset_list_Data = $('#sec_asset_list').DataTable({
        ...
        language: {
            "decimal": "",
            "info": showButtonsec ? "전체 _TOTAL_건 <button class='btn btn-outline-info me-1px' type='button' id='sec_all_select'>필터된 자산 그룹 생성</button>" : "전체 _TOTAL_건",
			//"info": "전체 _TOTAL_건 ",
            "infoEmpty": "데이터가 없습니다.",
...
//    const swVerHTML = "<ul>" + swVer2.map(item => "<li>" + item + "</li>").join("") + "</ul>";
    ...
    $(document).on('click', '#sec_all_select', function() {
        $.ajax({
            url: 'sec_select_all/',  // AJAX 요청을 보낼 URL (해당 URL에 파이썬 코드를 작성해야 합니다)
            type: 'POST',
            data: {
                data: sec_asset_list_Data.ajax.params(),  // 데이터 테이블의 현재 요청에 대한 파라미터 정보를 전달합니다
                filter: {
